<#
.SYNOPSIS
    Restrict user access and permissions granted through default Microsoft configurations.
.DESCRIPTION
    Restrict default guest and user permissions that allow self-service account creation, guest invitations, application consents, application creation, and more. Create Conditional Access Policy to restrict Microsoft Admin Portals.
.INPUTS
    None
.COMPONENT
    PowerShell Microsoft Graph PowerShell Module
.ROLE
    Sufficient rights in Azure/Entra AD
.FUNCTIONALITY
    Restrict default guest and user permissions that allow self-service account creation, guest invitations, application consents, application creation, and more. Create Conditional Access Policy to restrict Microsoft Admin Portals.
#>

# Update Dangerous Defaults permissions
<#
Reference Documents:
https://learn.microsoft.com/en-us/graph/api/authorizationpolicy-update?view=graph-rest-1.0&tabs=powershell
https://learn.microsoft.com/en-us/graph/api/resources/authorizationpolicy?view=graph-rest-1.0#allowinvitesfrom-values
https://learn.microsoft.com/en-us/graph/api/resources/defaultuserrolepermissions?view=graph-rest-1.0
#>

Function Invoke-PermissionRemediation {
    $body = @{
        allowEmailVerifiedUsersToJoinOrganization = $false
        allowInvitesFrom                          = 'adminsAndGuestInviters'
        allowUserConsentForRiskyApps              = $false
        blockMsolPowerShell                       = $true # May wish to comment out
        defaultUserRolePermissions                = @{
            allowedToCreateApps                      = $false
            allowedToCreateSecurityGroups            = $false
            allowedToReadBitlockerKeysForOwnedDevice = $true
            allowedToCreateTenants                   = $false
            permissionGrantPoliciesAssigned          = @("ManagePermissionGrantsForSelf.microsoft-user-default-low", "ManagePermissionGrantsForOwnedResource.microsoft-dynamically-managed-permissions-for-chat", "ManagePermissionGrantsForOwnedResource.microsoft-dynamically-managed-permissions-for-team") # May wish to comment out
        }
        guestUserRoleId                           = '2af84b1e-32c8-42b7-82bc-daa82404023b'
    }

    Invoke-GraphRequest -Method PATCH -Uri "https://graph.microsoft.com/beta/policies/AuthorizationPolicy" -Body $body
}


Function Create-ConditionalAccessPolicy {
    # Restrict Admin Portal Access via Conditional Access Policies
    <#
    Roles Targeted in Policy
    ...
    $body = @'
        {
            "displayName": "Restrict Admin Portals",
...
'@
    ...
    Invoke-GraphRequest -Method POST -Uri "https://graph.microsoft.com/beta//identity/conditionalAccess/policies" -Body $body
}

Invoke-PermissionRemediation
